# Building Facility Booking System - User Stories
...
As a resident, I want to receive an activation email with a link to set my password, so that I can access my personal account securely.

### US-2: Resident login

As a resident, I want to log into my personal account using email or phone + password, so that I can book the washing machine and manage my bookings.

### US-3: Admin login

As an administrator, I want to log in using username + password + two-factor authentication, so that I can securely access the admin panel.
...
As an administrator, I want to delete a resident account when they move out, so that former residents no longer have access to the system.

## 2. Booking Calendar & Viewing Availability
...
As a resident, I want the system to notify me if someone else booked the slot before I confirmed, so that I don't create an invalid or duplicate booking.
...
As a resident, I want to receive an email confirming my booking, so that I know the reservation was successful.

### US-23: Resident receives account deletion email

As a resident, I want to receive an email when my account is deleted, so that I know I no longer have access.
